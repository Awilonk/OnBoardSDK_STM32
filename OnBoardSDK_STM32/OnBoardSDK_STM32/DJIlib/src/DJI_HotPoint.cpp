...
//              callback ? callback : startCallback, 500, 2);
		   api->send(2, 0, SET_MISSION, CODE_HOTPOINT_START, &data, sizeof(data),
              callback ? callback : startCallback, 500, 2);
			 //UNENCRYPTED
...
//              callback ? callback : commonCallback, 500, 2);
    api->send(2, 0, SET_MISSION, CODE_HOTPOINT_STOP, &zero, sizeof(zero),
              callback ? callback : commonCallback, 500, 2);
		//UNENCRYPTED
}
...
//              callback ? callback : commonCallback, 500, 2);
		 api->send(2, 0, SET_MISSION, CODE_HOTPOINT_PALSTANCE, &Data, sizeof(Data),
              callback ? callback : commonCallback, 500, 2);
		 //UNENCRYPTED
}
...
//              callback ? callback : commonCallback,userData);
		api->send(2, 0, SET_MISSION, CODE_HOTPOINT_RADIUS, &degree, sizeof(degree), 500, 2,
              callback ? callback : commonCallback,userData);
}
...
{
    uint8_t zero = 0;
    api->send(2, 0, SET_MISSION, CODE_HOTPOINT_SETYAW, &zero, sizeof(zero), 500,
...
{
    uint8_t zero = 0;
    api->send(2, 0, SET_MISSION, CODE_HOTPOINT_LOAD, &zero, sizeof(zero), 500,
              2, callback ? callback : commonCallback, userData);
...
void HotPoint::setPalstance(float32_t defree) { data.palstance = defree; }  //what's the units?
